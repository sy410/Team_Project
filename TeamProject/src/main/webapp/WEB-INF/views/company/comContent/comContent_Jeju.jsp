...
/* 	표시 지우기			var marker = new daum.maps.Marker({ map: map, position: coords }); 
   ...
   <section class="py-5 bg-gray-100 shadow">
     <div class="container">
       <h1>제주특별자치도</h1><h4 class="text-gray-600">Jeju</h4> 
       <p class="lead mb-5 text-gray-600 fs-5">섬 전체가 하나의 거대한 관광자원인 제주도. 에메랄드빛 물빛이 인상적인 협재 해수욕장은 제주 대표 여행지며, 파도가 넘보는 주상절리와 바다 위 산책로인 용머리 해안은 제주에서만 볼 수 있는 천혜의 자연경관으로 손꼽힙니다. 드라마 촬영지로 알려진 섭지코지는 꾸준한 사랑을 받고 있으며 한라봉과 흑돼지, 은갈치 등은 제주를 대표하는 음식입니다.</p>
       <ul class="nav nav-pills-custom">
         <li class="nav-item"><a class="nav-link" href="#">제주시</a></li>
         <li class="nav-item"><a class="nav-link" href="#">서귀포</a></li>
         <li class="nav-item"><a class="nav-link" href="#">애월</a></li>
       </ul>
     </div>
   </section>
   <section class="py-5">
...
<%--           <c:if test=${fn:substring(list.addr,0,2) eq '서울'}> --%>       
	...
	<c:if test="${pageMaker.prev && pageMaker.spageNo > 1}">
		<li class="page-item"><a class="page-link" href="ccontent_jeju${pageMaker.searchQuery(1)}"><<</a></li>
		<li class="page-item"><a class="page-link" href="ccontent_jeju${pageMaker.searchQuery(pageMaker.spageNo-1)}"><i class="fa fa-angle-left"></i></a></li>
	</c:if>
	...
	<c:forEach var="i" begin="${pageMaker.spageNo}" end="${pageMaker.epageNo}">
		...
		<c:if test ="${i != pageMaker.cri.currPage }">
			<li class="page-item"><a class="page-link" href ="ccontent_jeju${pageMaker.searchQuery(i)}">${i}</a></li>
		</c:if>
	</c:forEach>
	
	<!-- 3) Next > , Last >> 처리 -->
	<c:if test="${pageMaker.next && pageMaker.epageNo > 0}">
		<li class="page-item"><a class="page-link" href="ccontent_jeju${pageMaker.searchQuery(pageMaker.epageNo+1)}"><i class="fa fa-angle-right"></i></a></li>
		<li class="page-item"><a class="page-link" href="ccontent_jeju${pageMaker.searchQuery(pageMaker.lastPageNo)}">>></a></li>
	</c:if>
   </ul>
  </nav>
     </div>
