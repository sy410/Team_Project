...
<script>
 ...
 <nav class="navbar navbar-expand-lg fixed-top shadow navbar-light bg-white">
   <div class="container-fluid">
    ...
    <!-- Hero Section-->
    <section class="py-5 hero py-6 py-lg-7 text-white dark-overlay"><img class="bg-image" src="resources/img/review.jpg" alt="????" >
      ...
      <div class="container">
		  ...
		  <div id="searchBar" style="display: inline-block; margin: 0 5px; float: right;"><form class="d-flex">  
			<select name="searchType" id="searchType" class="selectpicker me-3 mb-3 mb-lg-0" data-style="btn-selectpicker">
				...
				<option value="ci" <c:out value="${pageMaker.cri.searchType=='ci' ? 'selected':''}"/> >city</option>
				<option value="cw" <c:out value="${pageMaker.cri.searchType=='cw' ? 'selected':''}"/> >Content & ID</option>
				<option value="ct" <c:out value="${pageMaker.cri.searchType=='ct' ? 'selected':''}"/> >Content & Title</option>
			 </select>
                      ...
                      <table class="table">
                       ...
                       <div align="right">
                       	<c:if test="${loginID!=null && loginID!='admin'}">
                		 		<li class="nav-item mt-3 mt-lg-0 ml-lg-3 d-lg-none d-xl-inline-block"><a class="btn btn-outline-primary" href="rinsertf">글쓰기</a></li>
         				</c:if>
	...
	<c:if test="${pageMaker.prev && pageMaker.spageNo > 1}">
		<li class="page-item"><a class="page-link" href="rlist${pageMaker.searchQuery(1)}"><<</a></li>
		<li class="page-item"><a class="page-link" href="rlist${pageMaker.searchQuery(pageMaker.spageNo-1)}"><i class="fa fa-angle-left"></i></a></li>
	</c:if>
	...
	<c:forEach var="i" begin="${pageMaker.spageNo}" end="${pageMaker.epageNo}">
		...
		<c:if test ="${i != pageMaker.cri.currPage }">
			<li class="page-item "><a class="page-link" href ="rlist${pageMaker.searchQuery(i)}">${i}</a></li>
		</c:if>
	</c:forEach>
	
	<!-- 3) Next > , Last >> 처리 -->
	<c:if test="${pageMaker.next && pageMaker.epageNo > 0}">
		<li class="page-item"><a class="page-link" href="rlist${pageMaker.searchQuery(pageMaker.epageNo+1)}"><i class="fa fa-angle-right"></i></a></li>
		<li class="page-item"><a class="page-link" href="rlist${pageMaker.searchQuery(pageMaker.lastPageNo)}">>></a></li>
	</c:if>
       </ul>
      </nav>
      </div>
